...
def collect_files_from_dir(data_dir, path_label, file_lists):
    """从一个包含 7 个 .job 文件的目录中收集文件到对应列表。"""
    contents = os.listdir(data_dir)
    if len(contents) < 7:
        return
    file_lists['prefix'].append(data_dir)
    file_lists['labels'].append(path_label)
    for file in contents:
        filepath = os.path.join(data_dir, file)
        if 'masked_indices_' in file:
            file_lists['masked_indices'].append(filepath)
        elif 'real_indices_' in file:
            file_lists['real_indices'].append(filepath)
        elif 'attention_mask_' in file:
            file_lists['attention_mask'].append(filepath)
        elif 'connect_comp_' in file:
            file_lists['connect_comp'].append(filepath)
        elif 'rna_type_' in file:
            file_lists['rna_type'].append(filepath)
        elif 'mask_' in file:
            file_lists['mask'].append(filepath)
        elif 'neighbor_gene_distribution_' in file:
            file_lists['neighbor_gene_distribution'].append(filepath)


def discover_files(train_paths):
    """自动发现训练数据文件，兼容两级和三级目录结构。

    两级结构: train_path/tokens-X/{masked_indices_*.job, ...}
    三级结构: train_path/sample/prefix/{masked_indices_*.job, ...}
    """
    file_lists = {
        'masked_indices': [], 'mask': [], 'real_indices': [],
        'attention_mask': [], 'connect_comp': [], 'rna_type': [],
        'neighbor_gene_distribution': [], 'prefix': [], 'labels': [],
    }
    ...
    for path in train_paths:
        ...
        for sample in os.listdir(path):
            ...
            if not os.path.isdir(sample_dir):
                continue
            # Check if data files are directly in sample_dir (2-level)
            sample_contents = os.listdir(sample_dir)
            if any('masked_indices_' in f for f in sample_contents):
                collect_files_from_dir(sample_dir, path_label, file_lists)
            else:
                # 3-level structure: look one level deeper
                for prefix in sample_contents:
                    prefix_dir = os.path.join(sample_dir, prefix)
                    if os.path.isdir(prefix_dir):
                        collect_files_from_dir(prefix_dir, path_label, file_lists)
...
def main(rank, args, config_train):
    ...
    file_lists = discover_files(train_path)
    masked_indices_files_list = file_lists['masked_indices']
    mask_files_list = file_lists['mask']
    real_indices_files_list = file_lists['real_indices']
    attention_mask_files_list = file_lists['attention_mask']
    connect_comp_files_list = file_lists['connect_comp']
    rna_type_files_list = file_lists['rna_type']
    neighbor_gene_distribution_files_list = file_lists['neighbor_gene_distribution']
    file_prefix_list = file_lists['prefix']
    file_labels_list = file_lists['labels']
