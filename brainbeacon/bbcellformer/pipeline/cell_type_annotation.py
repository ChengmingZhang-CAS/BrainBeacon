    ...
    with torch.no_grad():
        ...
        for i, data_dict in enumerate(dataloader):
            ...
            for j in range(0, len(idx), batch_size):
                ...
                # Always collect latent embeddings regardless of whether label exists
                latent.append(out_dict['latent'])
                logits.append(out_dict['logits'])
        ...
        if len(epoch_loss) == 0:
            return {'pred': pred, 'latent': latent, 'logits': logits}
        else:
...
class CellTypeAnnotationPipeline(Pipeline):
    ...
    def predict(self, adata: ad.AnnData,
        ...
        self.model.to(device)
        assert self.fitted, 'Cell type annotation pipeline does not support zero shot setting. Please fine-tune the model_raw on downstream datasets before inference.'
        if batch_gene_list is not None:
            raise NotImplementedError('Batch specific gene set is not implemented for cell type annotation pipeline. Please raise an issue on Github for further support.')
        adata = self.common_preprocess(adata, config['hvg'], covariate_fields, ensembl_auto_conversion)
        print(f'After filtering, {adata.shape[1]} genes remain.')
        ...
        result = inference(self.model, dataloader, None, device,
                  config['max_eval_batch_size'], self.eval_dict, order_required=True)
        return result
