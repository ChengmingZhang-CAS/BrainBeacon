...
def train_one_epoch(model, dataloader, optimizer, criterion, device, rank, writer, esm_embedding_map, global_step,
    ...
    for micro_step, (masked_indices, mask, real_indices, attention_mask, connect_comp, rna_type, neighbor_gene_distribution) in \
        ...
        # Neutralize the local expression-deviation input at MGP target positions.
        neighbor_gene_distribution = neighbor_gene_distribution.masked_fill(mask == 0, 0)

        masked_indices_view = masked_indices.view(-1).long()
        esm_embedding = torch.index_select(esm_embedding_map, dim=0, index=masked_indices_view)
        esm_embedding = esm_embedding.view(masked_indices.shape[0], masked_indices.shape[1], esm_embedding.shape[-1])
