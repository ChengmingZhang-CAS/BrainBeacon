...
def _get_gene_values(adata, key):
    if key == "var_names":
        return pd.Series(adata.var_names, dtype="string").fillna("").str.strip().to_numpy()
    if key in adata.var:
        return adata.var[key].astype("string").fillna("").str.strip().to_numpy()
    return None


def valid_homo_value(value):
    text = str(value).strip()
    return text and text.lower() not in {"nan", "none", "null", "-1"}


def build_gene_dict_maps(gene_dict, homo_col="homo_connect_id"):
    """Build species-specific gene-id to homo-group lookup tables from gene_dict."""
    if homo_col not in gene_dict.var:
        raise KeyError(f"gene_dict.var missing '{homo_col}'.")
    var = gene_dict.var.copy()
    if "species" not in var:
        raise KeyError("gene_dict.var must contain species.")
    var["species"] = var["species"].astype(str)

    maps = {}
    for species in sorted(var["species"].str.lower().unique()):
        sub = var[var["species"].str.lower() == species]
        lookup = {}
        value_sources = {"var_names": pd.Series(sub.index.astype(str), index=sub.index)}
        for col in ["ensembl_id", "gene_id", "gene_symbol", "genenames"]:
            if col in sub:
                value_sources[col] = sub[col].astype(str)
        for values in value_sources.values():
            for key, homo in zip(values.astype(str), sub[homo_col].astype(str)):
                key = str(key).strip().upper()
                if key and key.lower() not in {"nan", "none", "null"} and valid_homo_value(homo):
                    lookup.setdefault(key, str(homo).strip())
        maps[species] = lookup
    return maps


def homo_ids_for_adata(adata, species, gene_maps):
    """Map an AnnData var table to homo_connect_id values using multiple gene id columns."""
    values_by_key = {"var_names": _get_gene_values(adata, "var_names")}
    for col in ["ensembl_id", "gene_id", "gene_symbol", "genenames"]:
        values = _get_gene_values(adata, col)
        if values is not None:
            values_by_key[col] = values

    species_key = str(species).lower()
    if species_key not in gene_maps:
        raise KeyError(f"Unsupported species for gene_dict mapping: {species}")
    lookup = gene_maps[species_key]

    homo_ids = np.array([None] * adata.n_vars, dtype=object)
    for idx in range(adata.n_vars):
        for values in values_by_key.values():
            value = str(values[idx]).strip().upper()
            homo = lookup.get(value)
            if homo is not None:
                homo_ids[idx] = homo
                break
    return homo_ids
...
def annotate_homo_ids(adata, species, gene_maps, homo_col="homo_connect_id"):
    """Return a copy with var[homo_col] filled from existing values or gene_dict maps."""
    if homo_col in adata.var:
        homo_ids = adata.var[homo_col].astype("string").fillna("").str.strip().to_numpy()
        homo_ids = np.asarray([x if valid_homo_value(x) else None for x in homo_ids], dtype=object)
    else:
        homo_ids = homo_ids_for_adata(adata, species, gene_maps)
    out = adata.copy()
    out.var[homo_col] = homo_ids
    return out


def filter_all_by_common_homo(adata_by_name, species_by_name, gene_dict, homo_col="homo_connect_id"):
    """Annotate and filter datasets to homo groups shared by all datasets.

    Genes sharing the same homo group are all retained; the function does not collapse
    many-to-one or many-to-many homolog groups.
    """
    gene_maps = build_gene_dict_maps(gene_dict, homo_col=homo_col)
    annotated = {}
    homo_sets = {}
    for name, adata in adata_by_name.items():
        species = species_by_name[name]
        if isinstance(species, dict):
            species = species["species"]
        out = annotate_homo_ids(adata, species, gene_maps, homo_col=homo_col)
        keep_mapped = np.asarray([x is not None for x in out.var[homo_col].to_numpy()], dtype=bool)
        out = out[:, keep_mapped].copy()
        annotated[name] = out
        homo_sets[name] = set(out.var[homo_col].astype(str))

    common_homo = sorted(set.intersection(*homo_sets.values()))
    if not common_homo:
        raise ValueError("No common homo_connect_id groups across datasets.")
    common_set = set(common_homo)

    aligned = {}
    summary_rows = []
    count_rows = []
    for name, adata in annotated.items():
        before = int(adata_by_name[name].n_vars)
        mapped = int(adata.n_vars)
        keep = adata.var[homo_col].astype(str).isin(common_set).to_numpy()
        out = adata[:, keep].copy()
        aligned[name] = out

        counts = out.var[homo_col].astype(str).value_counts()
        for homo, count in counts.items():
            count_rows.append({"dataset": name, homo_col: homo, "n_genes": int(count)})
        summary_rows.append(
            {
                "dataset": name,
                "n_vars_before_homo": before,
                "n_mapped_homo_genes": mapped,
                "n_unique_homo_mapped": int(len(homo_sets[name])),
                "n_common_homo": int(len(common_homo)),
                "n_vars_after_homo": int(out.n_vars),
            }
        )

    return aligned, pd.DataFrame(summary_rows), pd.DataFrame(count_rows)


def mean_vector_for_adata(adata, gene_dict, mean_matrix):
    """Align a gene_dict mean vector to adata.var_names."""
...
def corrected_expression_matrix(adata, mean_vector):
    """Library-size normalize to 1e4 and divide by an aligned gene mean vector."""
    x = adata.X
    if sparse.issparse(x):
        x = sf_normalize(x.astype(np.float32).tocsr())
        inv_mean = np.zeros_like(mean_vector, dtype=np.float32)
        ok = mean_vector > 0
        inv_mean[ok] = 1.0 / mean_vector[ok]
        return x.multiply(inv_mean).tocsr()
    x = sf_normalize(np.asarray(x, dtype=np.float32))
    ok = mean_vector > 0
...
def compute_ref_homo_marker_scores(
    ...
    labels = ref.obs[label_col].astype(str).to_numpy()
    ...
    if sparse.issparse(x):
        total_sum = _as_1d(x.sum(axis=0)).astype(np.float32)
        for label in unique_labels:
            idx = labels == label
            n_in = int(idx.sum())
            n_rest = n_cells - n_in
            if n_in == 0 or n_rest == 0:
                continue
            in_sum = _as_1d(x[idx].sum(axis=0)).astype(np.float32)
            class_mean_by_label[label] = (in_sum / n_in).astype(np.float32, copy=False)
            rest_mean_by_label[label] = ((total_sum - in_sum) / n_rest).astype(np.float32, copy=False)
    else:
        total_sum = x.sum(axis=0, dtype=np.float32)
        for label in unique_labels:
            idx = labels == label
            n_in = int(idx.sum())
            n_rest = n_cells - n_in
            if n_in == 0 or n_rest == 0:
                continue
            in_sum = x[idx].sum(axis=0, dtype=np.float32)
            class_mean_by_label[label] = (in_sum / n_in).astype(np.float32, copy=False)
            rest_mean_by_label[label] = ((total_sum - in_sum) / n_rest).astype(np.float32, copy=False)

    if not class_mean_by_label:
        raise ValueError("No valid labels for marker scoring.")

    marker_labels = list(class_mean_by_label)
    class_mean = np.vstack([class_mean_by_label[label] for label in marker_labels])
    selected_parts = []
    for i, label in enumerate(marker_labels):
        nearest_other = np.zeros(ref.n_vars, dtype=np.float32) if len(marker_labels) == 1 else np.max(np.delete(class_mean, i, axis=0), axis=0)
        margin = (class_mean[i] - nearest_other).astype(np.float32, copy=False)
        rest = (class_mean[i] - rest_mean_by_label[label]).astype(np.float32, copy=False)
        one = pd.DataFrame(
            {
                "gene": ref.var_names.astype(str),
                homo_col: ref.var[homo_col].astype(str).to_numpy(),
                "mean_stereo": mean_vector,
                "marker_score_margin_second": margin,
                "marker_score_vs_rest_mean": rest,
                "marker_label": label,
            }
        )
        one = one[one[homo_col].isin(available_homo)].copy()
        ...
        one = one[np.isfinite(one[score_col]) & (one[score_col] > float(min_score))].copy()
        one["marker_score_active"] = one[score_col].astype(float)
        one.sort_values(["marker_score_active", "gene"], ascending=[False, True], inplace=True)
        one = one.drop_duplicates(homo_col, keep="first")
        if topk_per_label and topk_per_label > 0:
            one = one.head(topk_per_label).copy()
        one["label_marker_rank"] = np.arange(1, len(one) + 1)
        selected_parts.append(one)

    per_label_selected = pd.concat(selected_parts, ignore_index=True) if selected_parts else pd.DataFrame()
    if per_label_selected.empty:
        raise ValueError("Per-label marker selection produced no homo groups.")
    ...
    labels_by_homo = per_label_selected.groupby(homo_col)["marker_label"].apply(lambda x: ";".join(sorted(set(map(str, x)))))
...
def sample_cells_by_label(adata, label_col, max_cells_per_label, seed=0):
    """Randomly cap cells per label and return (adata_subset, n_dropped)."""
    ...
    return adata[keep_idx].copy(), int(adata.n_obs - len(keep_idx))


def sample_cells_by_fraction(adata, fraction, seed=0):
    """Randomly sample a fraction of cells and return (adata_subset, n_dropped)."""
    ...
    if fraction >= 1:
        return adata, 0
    rng = np.random.default_rng(seed)
    n_keep = max(1, int(round(adata.n_obs * fraction)))
    keep_idx = np.sort(rng.choice(adata.n_obs, size=n_keep, replace=False))
    return adata[keep_idx].copy(), int(adata.n_obs - len(keep_idx))
...
def plot_spatial_comparison(
    ...
    - Optionally save a true-label-normalized confusion matrix.
    """
    ...
    if save_confusion and has_true and has_pred:
