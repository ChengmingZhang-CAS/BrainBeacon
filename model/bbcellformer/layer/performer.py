...
class PerformerLayer(nn.Module, AbstractTrasnformerLayer):
    ...
    def _sa_block(self, x: Tensor, attn_mask: Optional[Tensor], output_attentions: bool = False):
        x = x.unsqueeze(0)
        # out, attn_weights = self.self_attn(x, attn_mask=attn_mask, output_attentions=output_attentions)
        out, attn_weights = self.self_attn(x, mask=attn_mask, output_attentions=output_attentions)
        return out[0], attn_weights

    def forward(self, x, attn_mask=None, output_attentions=False):
        if self.norm_first:
            x_prime, attn = self._sa_block(self.norm1(x), attn_mask, output_attentions=output_attentions)
            x = x + x_prime
            x = x + self._ff_block(self.norm2(x))
        else:
            x_prime, attn = self._sa_block(x, attn_mask, output_attentions=output_attentions)
