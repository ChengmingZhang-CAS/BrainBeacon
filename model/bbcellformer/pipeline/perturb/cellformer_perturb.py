...
class OmicsFormerPerturb(nn.Module):
    def __init__(self, gene_list, enc_mod, enc_hid, enc_layers, post_latent_dim, dec_mod, dec_hid, dec_layers,
                 ...
                 mask_feature_rate=0.8, drop_node_rate=0., max_batch_size=5000, cat_dim=None, conti_dim=None,
                 pe_type='sin', cat_pe=True, gene_emb=None, latent_mod='vae', w_li=1., w_en=1., w_ce=1.,
                 head_type=None, dsbn=False, ecs=False, dar=False, input_covariate=False,
                 num_clusters=16, dae=True, lamda=0.5, mask_beta=False, use_perturbation=False, 
                 num_perturb_conditions=None, gene_embeddings=None, symbol_to_emb_idx=None, condition_to_id=None, case_insensitive_mapping=None, **kwargs):
        super(OmicsFormerPerturb, self).__init__()

        self.embedder = OmicsEmbeddingLayerPerturb(gene_list, enc_hid, norm, activation, model_dropout,
                                                   pe_type, cat_pe, gene_emb, inject_covariate=input_covariate, 
                                                   batch_num=batch_num, use_perturbation=use_perturbation, 
        ...
        if dsbn:
            ...
            # self.bb_norm = PreLatentNorm('dsbn', enc_hid, dataset_num)
            self.bb_norm = nn.LayerNorm(enc_hid)
        else:
            self.pre_latent_norm = PreLatentNorm('ln', enc_hid)
            # self.bb_norm = PreLatentNorm('ln', enc_hid)
            self.bb_norm = nn.LayerNorm(enc_hid)
        # self.post_latent_norm = nn.LayerNorm(post_latent_dim, dataset_num)

    def forward(self, x_dict, input_gene_list=None, d_iter=False, bb_weight=1.0):
        if self.mask_type == 'input':
            ...
            x_dict['h'] = (1 - bb_weight) * x_dict['h'] + bb_weight * bb_emb
            ...
            if input_gene_list is not None:
                gene_idx = torch.tensor([self.embedder.feat_enc.gene_index[o] for o in input_gene_list if
                                         o in self.embedder.feat_enc.gene_index]).long()
                x_dict['input_gene_mask'] = gene_idx
            else:
                if x_dict['x_seq'].shape[1] != len(self.pretrained_gene_list):
                    raise ValueError(
                        'The input gene size is not the same as the pretrained gene list. Please provide the input gene list.')
                gene_idx = torch.arange(x_dict['x_seq'].shape[1]).long()
                x_dict['input_gene_mask'] = gene_idx
            x_dict['h'] = self.encoder(x_dict)['hidden']
        else:
            raise NotImplementedError(f"Only support mask_type in ['input', 'hidden'], but got {self.mask_type}")
        ...
        x_dict["dataset"] = torch.full((x_dict['h'].shape[0],), 0, device=device, dtype=torch.long)
        x_dict["platform"] = torch.full((x_dict['h'].shape[0],), 0, device=device, dtype=torch.long)
        if d_iter:
