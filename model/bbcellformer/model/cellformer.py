...
class OmicsFormer(nn.Module):
    ...
    def forward(self, x_dict, input_gene_list=None, d_iter=False, bb_weight=0.5, output_attentions=False):
        if self.mask_type == 'input':
            ...
            if output_attentions:
                encoder_output = self.encoder(x_dict, output_attentions=True)
                x_dict['h'] = encoder_output['hidden']
                encoder_attention = encoder_output.get('attn', None)
            else:
                x_dict['h'] = self.encoder(x_dict)['hidden']
        else:
        ...
        if d_iter:
            ...
            if output_attentions:
                out_dict['attention'] = encoder_attention
